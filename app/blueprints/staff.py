...
def _receipt_text_lines(order, settings):
    ...
    # Kolom karakter RAW ESC/POS ditentukan resolusi printhead-nya
    # (fisik), bukan setting lebar kertas per mm - printhead kelas 58mm
    # (baik yang disetel "48" ataupun "58") umumnya selalu 32 kolom di
    # font default, kelas 80mm ("72"/"80") umumnya 42 kolom. Beda dengan
    # receipt_pdf yang bisa atur presisi per-mm karena dia gambar vektor,
    # bukan teks monospace bawaan printer.
    width = 32 if settings.receipt_paper_width in ("48", "58") else 42
    lines = []
...
def _print_receipt_to_printer(order, settings):
    ...
    # Ketebalan cetak (receipt_print_weight) diterjemahkan ke perintah
    # ESC/POS standar - beda dengan receipt_pdf (PDF cuma "terlihat" tebal
    # di file-nya), ini benar-benar mengubah cara printhead menembak
    # panas, jadi memang membantu printer yang head-nya sudah aus/pudar:
    #   ESC E 1 / ESC E 0 - bold seluruh struk on/off
    #   ESC M 1 / ESC M 0 - Font B (lebih kecil/tipis) / Font A (normal)
    if settings.receipt_print_weight == "bold":
        body = b"\x1b\x45\x01" + body + b"\x1b\x45\x00"
    elif settings.receipt_print_weight == "thin":
        body = b"\x1b\x4d\x01" + body + b"\x1b\x4d\x00"
